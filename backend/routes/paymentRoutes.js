...
router.post('/wallet/create-order', async (req, res) => {
  try {
    ...
    // Create receipt tag (strictly under 40 characters)
...
router.post('/wallet/verify-payment', async (req, res) => {
  try {
    ...
    const { data: existingTransaction, error: checkError } = await db
      ...
      .select('id')
      .eq('user_id', userId)
      .match({ 'metadata->razorpay_payment_id': razorpayPaymentId })
      .maybeSingle();

    if (checkError) {
      console.error('[YogiDesk Wallet] Duplicate check error:', checkError.message);
    }

    if (existingTransaction?.id) {
      console.warn('[YogiDesk Wallet] Duplicate payment detected:', razorpayPaymentId);
    ...
    await db.from('wallet_transactions').insert([{
      ...
      if (error) {
        console.error('[YogiDesk Wallet] Transaction log failed:', error.message);
      }
    });
