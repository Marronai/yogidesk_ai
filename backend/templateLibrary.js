...
export const getTemplatesBySpecialty = (specialty) => {
  ...
  // Always merge with General marketing templates for all users
  return [...specific, ...MEDICAL_TEMPLATES.GENERAL];
};

export const calculateCampaignCost = (patientCount, templateType) => {
  // Utility Templates = ₹0.20 per message
  // Marketing Templates = ₹0.90 per message
  const unitPrice = templateType === 'UTILITY' ? 0.20 : 0.90;
  return (patientCount * unitPrice).toFixed(2);
};